				...
				u8 *addr, gfp_t gfp)
{
	...
	spin_lock_init(&sta->ps_lock);  //yangfh add.
...
void ieee80211_sta_ps_deliver_wakeup(struct sta_info *sta)
{
	...
	unsigned long flags, i = 0;  //yangfh add refer to backports-3.16.1.
	struct sk_buff_head *tmp = NULL;
	...
	spin_lock(&sta->ps_lock);  //yangfh add refer to backports-3.16.1.
	/* Send all buffered frames to the station */
	for (ac = 0; ac < IEEE80211_NUM_ACS; ac++) {
		int count = skb_queue_len(&pending), tmp;

		spin_lock_irqsave(&sta->tx_filtered[ac].lock, flags);  //yangfh add refer to backports-3.16.1.
		skb_queue_splice_tail_init(&sta->tx_filtered[ac], &pending);
		spin_unlock_irqrestore(&sta->tx_filtered[ac].lock, flags);  //yangfh add refer to backports-3.16.1.
		tmp = skb_queue_len(&pending);
		filtered += tmp - count;
		count = tmp;

		spin_lock_irqsave(&sta->ps_tx_buf[ac].lock, flags);  //yangfh add refer to backports-3.16.1.
		skb_queue_splice_tail_init(&sta->ps_tx_buf[ac], &pending);
		spin_unlock_irqrestore(&sta->ps_tx_buf[ac].lock, flags); //yangfh add refer to backports-3.16.1.
		tmp = skb_queue_len(&pending);
		buffered += tmp - count;
	}
	ieee80211_add_pending_skbs_fn(local, &pending, clear_sta_ps_flags, sta);
	spin_unlock(&sta->ps_lock); //yangfh add refer to backports-3.16.1.
