...
static int nl80211_set_wiphy(struct sk_buff *skb, struct genl_info *info)
{
	...
	if (changed) {
		...
		if (wdev) {
			old_retry_short = wdev->retry_short;
			old_retry_long = wdev->retry_long;
			old_rts_threshold = wdev->rts_threshold;
		}
		old_coverage_class = rdev->wiphy.coverage_class;

		if (changed & WIPHY_PARAM_RETRY_SHORT)
			wdev->retry_short = retry_short;
		if (changed & WIPHY_PARAM_RETRY_LONG)
			wdev->retry_long = retry_long;
		...
		result = rdev->ops->set_wiphy_params(&rdev->wiphy, netdev,
						     changed);
		if (result) {
			rdev->wiphy.frag_threshold = old_frag_threshold;
			if (wdev) {
				wdev->rts_threshold = old_rts_threshold;
				wdev->retry_short = old_retry_short;
				wdev->retry_long = old_retry_long;
			}
			      ...
			      struct net_device *dev)
{
	...
	NLA_PUT_U8(msg, NL80211_ATTR_WIPHY_RETRY_SHORT,
		   dev->ieee80211_ptr->retry_short);
	NLA_PUT_U8(msg, NL80211_ATTR_WIPHY_RETRY_LONG,
		   dev->ieee80211_ptr->retry_long);
...
static int nl80211_set_wowlan_aw(struct cfg80211_registered_device *rdev)
{
        ...
        printk(KERN_ERR "setting wowlan first time\n");
        if (!rdev->wiphy.wowlan.flags)
                return -EOPNOTSUPP;
...
static int nl80211_set_interface(struct sk_buff *skb, struct genl_info *info)
{
	...
	if (!err && params.use_4addr != -1)
        ...
        if(!rdev->wowinit){
                rdev->wowinit = 1;
                printk(KERN_ERR "calling nl80211_set_wowlan_cfg\n");
                err = nl80211_set_wowlan_aw(rdev);
        }
#endif
	return err;
}
...
#ifdef TES_P2P_0005_CLEAN_RESULT_AFTER_SCAN
	///w, TES_P2P_0005 WorkAround:
	///w, p2p sd query frame will be sent after there are some p2p peers in p2p find result .
	///w, However  the result maybe too old to make sure these p2p peer is in p2p listen
	///w, then p2p peer can't receiver the p2p sd query frame.
	///w, cleaning the result after getting result make the result is new as WorkAround
	///w, but it will cause some channel can't experence in ui. For Example : Channel 11
	cfg80211_bss_clean_after_scan(rdev);
#endif
...
static int nl80211_update_acm_up(struct sk_buff *skb, struct genl_info *info)
{
	struct cfg80211_registered_device *rdev = info->user_ptr[0];
	struct net_device *dev = info->user_ptr[1];
	u8 enable_up = 0;
	u8 update_up;
	if (!info->attrs[NL80211_ATTR_ENABLE_UP] ||
	    !info->attrs[NL80211_ATTR_ADMITTED_UP])
		return -EINVAL;
	enable_up = nla_get_u8(info->attrs[NL80211_ATTR_ENABLE_UP]);
	update_up = nla_get_u8(info->attrs[NL80211_ATTR_ADMITTED_UP]);
	if (!rdev->ops->update_acm_up)
		return -EOPNOTSUPP;

	return rdev->ops->update_acm_up(&rdev->wiphy, dev, enable_up,
					update_up);
}
...
#ifdef CONFIG_XRADIO_TESTMODE
	{
		.cmd = NL80211_CMD_UPDATE_ACM_UP,
		.doit = nl80211_update_acm_up,
		.policy = nl80211_policy,
		.flags = GENL_ADMIN_PERM,
		.internal_flags = NL80211_FLAG_NEED_NETDEV_UP |
				  NL80211_FLAG_NEED_RTNL,
			       ...
			       const u8 *ie, size_t ie_len, bool from_ap)
{
	...
	NLA_PUT_U32(msg, NL80211_ATTR_WIPHY, rdev->wiphy_idx);
	NLA_PUT_U32(msg, NL80211_ATTR_IFINDEX, netdev->ifindex);
	/* Implementation in Supplicant is such that for reason
	 * WLAN_REASON_UNSPECIFIED it will reboot the driver,
	 * in order to prevent this, reason is changed from
	 * WLAN_REASON_UNSPECIFIED to WLAN_REASON_DEAUTH_LEAVING.
	 */
	if (reason == WLAN_REASON_UNSPECIFIED)
		reason = WLAN_REASON_DEAUTH_LEAVING;
	if (from_ap && reason)
