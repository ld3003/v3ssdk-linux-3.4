			 ...
			 struct iw_param *rts, char *extra)
{
	...
	u32 orts = wdev->rts_threshold;
	int err;

	if (rts->disabled || !rts->fixed)
		wdev->rts_threshold = (u32) -1;
	else if (rts->value < 0)
		return -EINVAL;
	else
		wdev->rts_threshold = rts->value;

	err = rdev->ops->set_wiphy_params(wdev->wiphy, dev,
					  WIPHY_PARAM_RTS_THRESHOLD);
	if (err)
		wdev->rts_threshold = orts;
				  ...
				  struct iw_param *retry, char *extra)
{
	...
	struct cfg80211_registered_device *rdev = wiphy_to_dev(wdev->wiphy);
	u32 changed = 0;
	u8 olong = wdev->retry_long;
	u8 oshort = wdev->retry_short;
	int err;
	...
	if (retry->flags & IW_RETRY_LONG) {
		wdev->retry_long = retry->value;
		changed |= WIPHY_PARAM_RETRY_LONG;
	} else if (retry->flags & IW_RETRY_SHORT) {
		wdev->retry_short = retry->value;
	...
	err = rdev->ops->set_wiphy_params(wdev->wiphy, dev, changed);
	if (err) {
		wdev->retry_short = oshort;
		wdev->retry_long = olong;
	}
			   ...
			   struct iw_param *retry, char *extra)
{
	...
	if (retry->flags == 0 || (retry->flags & IW_RETRY_SHORT)) {
		...
		retry->value = wdev->retry_short;
		if (wdev->retry_long != wdev->retry_short)
