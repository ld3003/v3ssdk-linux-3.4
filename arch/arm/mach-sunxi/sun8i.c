...
/* Add by Harry for TSC2003 TS */
static struct i2c_board_info i2c_ts_devs[] __initdata = {
...
#endif
	/* Add by Harry */
	if (i2c_register_board_info(0, i2c_ts_devs, ARRAY_SIZE(i2c_ts_devs)) < 0) {
		printk("%s()%d - tsc2003 init failed!\n", __func__, __LINE__);
	}
	printk("tsc2003 device registered\n");
