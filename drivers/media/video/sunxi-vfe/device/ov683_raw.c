...
#define DEV_DBG_EN      1 
#if(DEV_DBG_EN == 1)    
#define vfe_dev_dbg(x,arg...) 	printk("[OV683_RAW]"x,##arg)
#else
#define vfe_dev_dbg(x,arg...) 
#endif
#define vfe_dev_err(x,arg...) 	printk("[OV683_RAW]"x,##arg)
#define vfe_dev_print(x,arg...) printk("[OV683_RAW]"x,##arg)
...
#define SENSOR_NAME "ov683_raw"

static int ov683_streamon = 0;
...
void stream_control(struct v4l2_subdev *sd,int on)
{
	...
	if(on != 0) {
		...
		while((reg_val_show(sd,0x150e) != 0x88)&&(i++ < 50)) {	
			msleep(10);
		}
 ...
 *****************************************************
 */
void read_sensor_reg(struct v4l2_subdev *sd, unsigned char id, unsigned short reg)
{
	int i = 0;
	reg_val_store(sd,0x1500,0xE4);	//cmd id
	reg_val_store(sd,0x1510,id);	//sensor id

	reg_val_store(sd,0x1511,reg >> 8);	//sensor addr_h
	reg_val_store(sd,0x1512,reg & 0xff);	//sensor addr_l
	reg_val_store(sd,0x150e,0x00);  // clean flag
	reg_val_store(sd,0x150f,0x80);  //issue cmd
...
void write_sensor_reg(struct v4l2_subdev *sd, unsigned char id, unsigned short reg, unsigned char value)
{
	...
	reg_val_store(sd,0x1511,reg >> 8);	//sensor addr_h
	reg_val_store(sd,0x1512,reg & 0xff);	//sensor addr_l
	reg_val_store(sd,0x1513,value);  //sensor data
	reg_val_store(sd,0x150e,0x00);  // clean flag
	reg_val_store(sd,0x150f,0x80);  // issue cmd
...
void set_frame_rate(struct v4l2_subdev *sd, unsigned char fps)
{
	reg_val_store(sd,0x1500,0xE2);  // cmd id	
    reg_val_store(sd,0x1501,0x03);	//Data Size
	reg_val_store(sd,0x1502,0x10);	//Start Address
	reg_val_store(sd,0x1510,0x80);	//Set Frame rate
	reg_val_store(sd,0x1511,fps);  //Frame rate high 8 bit
	reg_val_store(sd,0x1512,0x00);  //Frame rate Low 8 bit
	reg_val_store(sd,0x150e,0x00);  // clean flag
	reg_val_store(sd,0x150f,0x80);  // issue cmd
}
...
//	unsigned char tmp = 0;
	...
	#ifdef DUAL_CAMERA
	reg_val_store(sd,0x1500,0xE4);	//cmd id
	reg_val_store(sd,0x1510,0x6C);	//sensor id

	reg_val_store(sd,0x1511,0x30);	//sensor addr_h
	reg_val_store(sd,0x1512,0x0B);	//sensor addr_l
	reg_val_store(sd,0x150e,0x00);  // clean flag
	reg_val_store(sd,0x150f,0x80);  //issue cmd
...
void write_ov683_reg(struct v4l2_subdev *sd)
{
	...
	reg_val_store(sd,0x1511,0x38);	//sensor addr_h
	reg_val_store(sd,0x1512,0x0e);	//sensor addr_l
	reg_val_store(sd,0x1513,0x12);  //sensor data
	reg_val_store(sd,0x150e,0x00);  // clean flag
	reg_val_store(sd,0x150f,0x80);  // issue cmd
	reg_val_show(sd,0x150e);  //read back
	msleep(10);
	...
	reg_val_store(sd,0x1511,0x38);	//sensor addr_h
	reg_val_store(sd,0x1512,0x0f);	//sensor addr_l
	reg_val_store(sd,0x1513,0x36);  //sensor data
	reg_val_store(sd,0x150e,0x00);  // clean flag
	reg_val_store(sd,0x150f,0x80);  // issue cmd
	reg_val_show(sd,0x150e);  //read back
	msleep(10);
...
#define PIO_BASE_O_T		  (0x01C20800+0xf0000000)
#define PC_CFG_BASE_T      ( PIO_BASE_O_T +0x48)
#define PC_DAT_BASE_T      ( PIO_BASE_O_T +0x58)

void debug_ov4689_power_cfg(void)
{
  unsigned int val = (1<<24);	
  writel((readl(PC_CFG_BASE_T)&(~(7<<24))|val),PC_CFG_BASE_T);
}

void debug_ov4689_power_on(int on )
{
	unsigned int val = (1<<6);	
	unsigned int tmp = readl(PC_DAT_BASE_T);
    writel(tmp&(~(1<<6)),PC_DAT_BASE_T);
}

//PC06  low
void ov683_set_gpio( )
{
	debug_ov4689_power_cfg();
	debug_ov4689_power_on(0);
}
...
//	write_sensor_reg(sd, 0x20, 0x380e, 0x12);
//	write_sensor_reg(sd, 0x20, 0x380f, 0x36);
//	write_sensor_reg(sd, 0x6C, 0x380e, 0x12);
//	write_sensor_reg(sd, 0x6C, 0x380f, 0x36);
//	set_frame_rate(sd, 10);
	...
	read_sensor_reg(sd, 0x20, 0x300A);
	read_sensor_reg(sd, 0x6C, 0x300B);
#endif		
	return 0;
}
