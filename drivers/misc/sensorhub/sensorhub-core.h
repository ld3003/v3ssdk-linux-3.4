...
#define SENSOR_HUB_RESET_CONTROLLED     4
#define SENSOR_HUB_DFU_IN_FLIGHT        16
#define SENSOR_HUB_DFU_COMPL            17
#define SENSOR_HUB_FLAGS_LAST           SENSOR_HUB_DFU_COMPL + 1

#define SENSOR_HUB_WAKE_LOCK_TIMEOUT    msecs_to_jiffies(200)
#define SENSOR_HUB_NUM_WAKE_UP_REPORTS  128

#define SH_TAG                          "[SENSORHUB] "
enum{
	DEBUG_ERR=1,
	DEBUG_ASSERT=1<<1,	
	DEBUG_MSG=1<<2,
...
/**
 * Get a property from the string metadata in DFU file
 * @param      metadata the metadata string
 * @param      prop     property name
 * @param[out] value    array to store property value in
 * @param      len      length of prop value array
 * @return          0 on success, -errno on error
 */
...
struct sensorhub_drv_data {
	...
	DECLARE_BITMAP(wake_reports, SENSOR_HUB_NUM_WAKE_UP_REPORTS);
#endif
...
struct sensorhub_transport_callbacks {
	 ...
	 * @return       0 on success, -errno otherwise
	 */
	int (* dfu_enterbl)(struct sensorhub_drv_data * ddata);

	/**
	 * Perform a firmware download to the device. The caller will ensure that
	 * dfu_enterbl has been called before this function.
	 *
	...
	/**
	 * DFU was interrupted and attempt to revert to application mode if possible.
	 ...
	 * @param len      Length of firmware
	 * @param metadata The metadata section
	 * @return       0 on success, -errno otherwise
	 */
...
static inline void sensorhub_notify_state(struct sensorhub_drv_data * ddata) {
	sysfs_notify(&ddata->dev->kobj, NULL, "state");
}

static inline void sensorhub_set_state(struct sensorhub_drv_data * ddata,
	                               enum sensorhub_state state) {
	extern const char * sensorhub_core_states[];
	dev_dbg(ddata->dev, "[%s] => [%s]\n", sensorhub_core_states[ddata->state],
			sensorhub_core_states[state]);
	ddata->state = state;
}

static inline void sensorhub_update_state(struct sensorhub_drv_data * ddata,
	                               enum sensorhub_state state) {
	sensorhub_set_state(ddata, state);
