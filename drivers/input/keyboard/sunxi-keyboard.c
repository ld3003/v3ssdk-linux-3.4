...
static void battery_adc_init(void)
{
	...
	type = script_get_item("bat_adc_para", "used", &script_val);
	if (SCIRPT_ITEM_VALUE_TYPE_INT != type) {
		return ;
	}
	use_adc_check_bat = script_val.val;

	type = script_get_item("bat_adc_para", "bat_max_vol", &script_val);
	if (SCIRPT_ITEM_VALUE_TYPE_INT == type) {
		bat_max_vol = script_val.val;
	}

	type = script_get_item("bat_adc_para", "bat_min_vol", &script_val);
	if (SCIRPT_ITEM_VALUE_TYPE_INT == type) {
		bat_min_vol = script_val.val;
	}
}
...
static ssize_t adc_get_data(struct device *dev, struct device_attribute *attr, char *buf)
{
	...
	if (use_adc_check_bat && bat_max_vol && bat_min_vol)
	{
		key_val = sunxi_keyboard_read_data(KEY_BASSADDRESS+LRADC_DATA0);
		printk("adc_val=%d\n", key_val);

		modify_vol = 1000 * key_val / 64 * 2;  // mv

		if (modify_vol > bat_max_vol)
			key_val = 100;
		else if (modify_vol < bat_min_vol)
			key_val = 0;
		else
			key_val = 100 * (modify_vol - bat_min_vol) /  (bat_max_vol - bat_min_vol);
	}
	printk("modify_vol=%d\n", modify_vol);
	return sprintf(buf, "%d\n", key_val);
}

static struct device_attribute keyboard_attributes[] = {
	__ATTR(data, 0444, adc_get_data, NULL),
};
#endif
...
#ifdef CONFIG_ARCH_SUN8IW8P1
	if (use_adc_check_bat)
		device_create_file(&sunxikbd_dev->dev, &keyboard_attributes[0]);
#endif
...
#ifdef CONFIG_ARCH_SUN8IW8P1
	if (use_adc_check_bat)
		device_remove_file(&sunxikbd_dev->dev, &keyboard_attributes[0]);
#endif
