...
#	cp -f rootfs.cpio.gz output/
	...
	#	printf "\n\033[0;31;1mUsing default config sun8iw8p1smp_tiger_spinand_standard_defconfig ...\033[0m\n\n"
# cp arch/arm/configs/${LICHEE_KERN_DEFCONF} .config
	# cp arch/arm/configs/sun8iw8p1smp_tiger_spinand_standard_defconfig .config
	printf "\n\033[0;31;1mUsing default linux config for RainbowPi ...\033[0m\n\n"	
	cp ${SDK_CONFIG_DIR}linux.config .config
	make ARCH=arm CROSS_COMPILE=${CROSS_COMPILE} oldconfig
    fi

    # YuanJun : binary
    cp drivers/mtd/spinand/sun8iw8p1/libnand_sun8iw8p1 drivers/mtd/spinand/sun8iw8p1/libnand_sun8iw8p1.a
    
    echo "Building uImage and modules."
    make ARCH=arm CROSS_COMPILE=${CROSS_COMPILE} -j6 uImage modules
    echo "Building uImage and modules done."
...
build_modules()
{
    ...
    if [ "x${NFS_DIR}" != "x" ]; then
	make ARCH=arm CROSS_COMPILE=${CROSS_COMPILE} INSTALL_MOD_PATH=${NFS_DIR} modules_install
    fi
